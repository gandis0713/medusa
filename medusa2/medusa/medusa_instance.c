...
static void print_memory_info()
{
    ...
    medusa_logi("Device Memory Information:");
    medusa_logi("  Total RAM: %llu MB", total_ram / (1024 * 1024));
    medusa_logi("  Free RAM:  %llu MB", free_ram / (1024 * 1024));
}
...
static bool try_device(const char* path, int* fd, const char* target)
{
    ...
    *fd = open(path, O_RDWR | O_CLOEXEC);
    if (*fd < 0)
    {
        medusa_loge("Opening %s failed: %s\n", path, strerror(errno));
        return false;
    }
    ...
    if (!version)
    {
        medusa_loge("Retrieving device version failed: %s\n", strerror(errno));
        goto fail;
    }
