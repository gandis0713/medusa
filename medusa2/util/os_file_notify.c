...
os_file_notifier_thread(void* data)
{
    ...
    while (!notifier->quit)
    {
        ...
        if (poll(fds, ARRAY_SIZE(fds), -1) == -1)
        {
            ...
            medusa_logw("Failed to poll on file notifier FDs: %s",
                        strerror(errno));
            return -1;
        }
        ...
        if (len == -1)
        {
            ...
            medusa_logw("Failed to read inotify events: %s", strerror(errno));
            return -1;
        }
        else
        {
                ...
                if (event->wd == notifier->dir_wd)
                {
                    ...
                    else if (strcmp(event->name, notifier->filename) == 0)
                    {
                        ...
                        if (event->mask & IN_CREATE)
                        {
                            ...
                            {
                                medusa_logw("Failed to add inotify watch for file");
                                return -1;
                            }
                        }
...
void os_file_notifier_destroy(os_file_notifier_t notifier)
{
    if (!notifier)
        return;
