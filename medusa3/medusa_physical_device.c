...
medusa_has_feature(struct medusa_physical_device* device, enum drm_v3d_param feature)
{
    ...
    if (v3d_ioctl(device->render_fd, DRM_IOCTL_V3D_GET_PARAM, &param) != 0)
        return false;
    return param.value;
}
...
void medusa_physical_device_init(struct medusa_physical_device* physical_device, struct medusa_instance* instance, int32_t render_fd, int32_t primary_fd)
{
    ...
    mesa_logi("Physical Device Features:");
    mesa_logi("  UIFCFG: %s", physical_device->fetures.UIFCFG ? "Yes" : "No");
    mesa_logi("  HUB_IDENT1: %s", physical_device->fetures.HUB_IDENT1 ? "Yes" : "No");
    mesa_logi("  HUB_IDENT2: %s", physical_device->fetures.HUB_IDENT2 ? "Yes" : "No");
    mesa_logi("  HUB_IDENT3: %s", physical_device->fetures.HUB_IDENT3 ? "Yes" : "No");
    mesa_logi("  CORE0_IDENT0: %s", physical_device->fetures.CORE0_IDENT0 ? "Yes" : "No");
    mesa_logi("  CORE0_IDENT1: %s", physical_device->fetures.CORE0_IDENT1 ? "Yes" : "No");
    mesa_logi("  CORE0_IDENT2: %s", physical_device->fetures.CORE0_IDENT2 ? "Yes" : "No");
    mesa_logi("  SUPPORTS_TFU: %s", physical_device->fetures.SUPPORTS_TFU ? "Yes" : "No");
    mesa_logi("  SUPPORTS_CSD: %s", physical_device->fetures.SUPPORTS_CSD ? "Yes" : "No");
    mesa_logi("  SUPPORTS_CACHE_FLUSH: %s", physical_device->fetures.SUPPORTS_CACHE_FLUSH ? "Yes" : "No");
    mesa_logi("  SUPPORTS_PERFMON: %s", physical_device->fetures.SUPPORTS_PERFMON ? "Yes" : "No");
    mesa_logi("  SUPPORTS_MULTISYNC_EXT: %s", physical_device->fetures.SUPPORTS_MULTISYNC_EXT ? "Yes" : "No");
    mesa_logi("  SUPPORTS_CPU_QUEUE: %s", physical_device->fetures.SUPPORTS_CPU_QUEUE ? "Yes" : "No");
